...
class BoardWrapper(HasTraits):
    ...
    def _mcu_changed(self):
        mcu = self.mcu
        s = [Define(
            name=k, value=v) for k, v in self.mcu.defines.as_dict().items()]
        s.sort(key=lambda x: x.name)
        self.defines = s
        self.digital_pins = [PinWrapper(pin=mcu.pin(x))
                             for x in mcu.pins.range_digital]
        self.analog_pins = [PinWrapper(pin=mcu.pin(x))
